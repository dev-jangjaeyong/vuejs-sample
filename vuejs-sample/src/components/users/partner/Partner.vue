<template>
  <div>
    <h4 class="font-weight-bold py-3 mb-4">
      <span class="text-muted font-weight-light">회원관리 /</span> 협력업체 관리
    </h4>
    ...
    <div class="ui-bordered px-4 pt-4 mb-4">
      <div class="form-row align-items-center">
        ...
        <div class="col-md mb-4">
          <label class="form-label">협력업체</label>
          <b-form-select v-model="searchParameters.affiliateCode" :options="affiliateCodeOptions">
            <template slot="first">
    ...
    <b-modal ref="modal" size="md" hide-footer>
      ...
      <partner-modal ref="userInfoModal" :user-info-modal-info="modalRowInfo" :flag="dupStatus" :user-status-code-options="userStatusCodeOptions" :gender-code-options="genderCodeOptions" :affiliate-code-options="affiliateCodeOptions" :mode="openModalMode"></partner-modal>
    </b-modal>
  </div>
</template>
...
export default {
  ...
  data: () => ({
    ...
    fields: [
      ...
      {key: 'createDt', sortable: true, tdClass: 'align-middle', label: '생성일자', 'class': 'text-center'},
  ...
  methods: {
    ...
    rowClicked (item, index, event) {
      ...
      this.openModalTitle = '협력업체 담당자 상세'
    ...
    openModal (flag) {
      if (flag) {
        // Row 등록 Modal Open
        this.openModalTitle = '협력업체 담당자 등록'
        this.openModalMode = 'POST'
        this.modalRowInfo = {
          affiliateCode: this.affiliateCodeOptions[0].value,
        ...
        // Row 수정 Model Open
        this.openModalTitle = '협력업체 담당자 정보 수정'
        this.openModalMode = 'PUT'
    ...
    saveModal () {
      ...
      if (this.openModalMode === 'POST') {
        ...
        this.$http.post('/partner/create', this.modalRowInfo)
          .then(
            (response) => {
    ...
    loadAffiliateCodeCombo () {
      const parameter = '?codeLv2=COMPANY_CODE&lv=3'
      this.loadCombo(parameter, 'affiliateCodeOptions')
    },

<template>
  <div>
    ...
    <b-table head-variant="dark"
             ...
             :items="items"
             :per-page="perPage "
             v-if="clickedChildrenInfo != undefined"
...
export default {
  ...
  computed: {
    ...
    totalSize: {
      get () { return (this.clickedChildrenInfo ? this.clickedChildrenInfo.length : 0) },
      set (value) {
      }
    }
  },
  ...
  methods: {
    rowClicked (row) {
      ...
      for (let index in this.clickedChildrenInfo) {
        const child = this.clickedChildrenInfo[index]
    ...
    getItems () {
      this.setOffset()
      if (this.clickedChildrenInfo) {
        if (this.clickedChildrenInfo.length > 0) {
          this.items = this.clickedChildrenInfo.filter((value, index) => this.offset <= index).filter((value, index) => index <= this.offset + this.perPage)
        }
      }
    },

<template>
  <div>
    <h4 class="font-weight-bold py-3 mb-4">
      <span class="text-muted font-weight-light">Template /</span> Code - Template 1
    </h4>
  ...
  methods: {
    ...
    openModal (codeNo) {
      if (codeNo) {
        ...
        this.modalCodeInfo = this.$commons.clone(this.clickedCodeInfo)
      } else {
        this.modalOpenMode = 'POST'
        this.modalCodeInfo = {
          upCodeNo: this.clickedCodeInfo.codeNo
        }
      }
    ...
    saveModal (evt) {
      ...
      if (!this.modalCodeInfo.codeName) {
        alert('코드명을 입력하세요.')
      } else if (!this.modalCodeInfo.codeValue) {
        alert('코드값을 입력하세요.')
      } else {
        if (confirm('저장하시겠습니까?')) {
          this.saveProc()
